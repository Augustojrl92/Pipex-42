...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/02 18:04:39 by aurodrig          #+#    #+#             */
/*   Updated: 2024/10/13 15:40:25 by aurodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_get_env_paths(char **env, t_context *context)
{
	...
	while (env && env[i])
	{
		if (ft_strnstr(env[i], "PATH=", 5))
		{
			...
			if (!context->env_path)
				ft_free_exit("Memory failed", NULL, 1, context);
			break ;
		}
		i++;
	}
...
void	execute_command(t_context *context, char *command, char **env)
{
	...
	if (!command_flags || command_flags[0] == NULL)
	{
		...
		ft_free_exit("Memory allocation failed", NULL, 1, context);
	}
			...
			|| command_flags[0][0] == '/'))
	{
		...
		ft_free_matrix(command_flags);
		ft_free_exit("Execution failed command: ", command, 127, context);
	}
	else if (ft_strchr(command_flags[0], '/') != NULL)
	{
		ft_free_matrix(command_flags);
		ft_free_exit("Invalid command: ", command, 127, context);
	}
	else
...
void	ft_get_path_comand(t_context *context, char *command, char **env)
{
	...
	if (!command_flags || command_flags[0] == NULL)
	{
		if (command_flags)
			ft_free_matrix(command_flags);
		ft_free_exit("Memory failed in get path", NULL, 1, context);
	}
	...
	if (full_path != NULL)
	{
		...
		free(full_path);
		ft_free_matrix(command_flags);
		ft_free_exit("Failed for command: ", command, 127, context);
	}
	ft_free_matrix(command_flags);
	ft_free_exit("Command not found: ", command, 127, context);
}
